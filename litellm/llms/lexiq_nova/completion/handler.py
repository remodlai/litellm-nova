...
class LexiqNovaError(Exception):
...
# check if lexiq_nova is installed
def validate_environment(model: str):
    ...
    try:
        ...
        raise LexiqNovaError(status_code=0, message=str(e))
...
def completion(
    ...
    if llm:
        ...
        raise LexiqNovaError(
            status_code=0, message="Need to pass in a model name to initialize Lexiq Nova"
        )
...
def batch_completions(
    ...
    responses = batch_completion(
        model="lexiq_nova/facebook/opt-125m",
        messages = [
            [
                {
                    "role": "user",
    ...
    try:
        ...
        error_str = str(e)
        raise LexiqNovaError(status_code=0, message=error_str)
    sampling_params = SamplingParams(**optional_params)
    prompts = []
    if model in custom_prompt_dict:
        # check if the model has a registered custom prompt
