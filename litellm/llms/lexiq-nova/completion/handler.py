...
# check if lexiq-nova is installed
def validate_environment(model: str):
    global llm
    try:
        from nova import LLM, SamplingParams  # type: ignore

        if llm is None:
            llm = LLM(model=model)
        return llm, SamplingParams
    except Exception as e:
        raise NovaError(status_code=0, message=str(e))
...
def batch_completions(
    ...
    try:
        ...
        error_str = str(e)
        raise NovaError(status_code=0, message=error_str)
    sampling_params = SamplingParams(**optional_params)
    prompts = []
    if model in custom_prompt_dict:
        # check if the model has a registered custom prompt
